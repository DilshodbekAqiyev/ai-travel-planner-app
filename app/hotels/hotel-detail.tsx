...
export default function HotelDetail() {
  ...
  const hotel: SearchHotelDetailsProps = JSON.parse(params.hotel as string);
  // console.log("listofhotels:", Hotels, typeof Hotels);
  ...
  return (
    <ScrollView style={styles.scrollContainer}>
      <View style={styles.container}>
        ...
        {/* Hotel Image */}
        <Image source={{ uri: `https://maps.googleapis.com/maps/api/place/photo?maxwidth=400&photo_reference=${hotel.photos[0].photo_reference}&key=${process.env.EXPO_PUBLIC_GOOGLE_MAP_KEY}` }} style={styles.mainImage} />


        {/* Hotel Name & Location */}
        <Text style={styles.title}>{hotel.name}</Text>
        <Text style={styles.location}>{hotel.formatted_address}</Text>
        ...
        {/* Hashtag */}
        <Text style={styles.hashtag}>{hotel.formatted_address.split(",")[0]}</Text>
        ...
        <TouchableOpacity onPress={() => Linking.openURL("https://goo.gl/maps/Xyz")}>
          <Text style={styles.linkText}>{hotel.formatted_address}</Text>
        </TouchableOpacity>
